...
void ThreadNtpSamples(void* parg) {
    ...
    RenameThread("BULLY-ntp-samples");
    ...
    while (!fShutdown) {
        ...
        if (GetNodesOffset() == INT_MAX && abs64(nNtpOffset) > 40 * 60)
        {
            ...
            std::string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong BULLY will not work properly.");
            strMiscWarning = strMessage;
            printf("*** %s\n", strMessage.c_str());
            uiInterface.ThreadSafeMessageBox(strMessage+" ", std::string("BULLY"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
        }
