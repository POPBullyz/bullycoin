...
bool CDBEnv::Open(boost::filesystem::path pathEnv_)
{
    ...
    if (!dbenv.get_lk_max_locks(&nMaxLocks))
    {
        ...
        {
            if (nBlocks < 1)
                strMessage = strprintf(_("Warning: DB_CONFIG has set_lk_max_locks %lu, which may be too low for a single block. If this limit is reached, BULLY may stop working."), (unsigned long)nMaxLocks);
            else
                strMessage = strprintf(_("Warning: DB_CONFIG has set_lk_max_locks %lu, which may be too low for a common blockchain reorganization. If this limit is reached, BULLY may stop working."), (unsigned long)nMaxLocks);
