...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("BULLY version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  BULLYd [options]                     " + "\n" +
                  "  BULLYd [options] <command> [params]  " + _("Send command to -server or BULLYd") + "\n" +
                  "  BULLYd [options] help                " + _("List commands") + "\n" +
                  "  BULLYd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "BULLY:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("BULLY"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("BULLY"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: BULLY.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: BULLYd.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 39377 or testnet: 19377)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 29375 or testnet: 19375)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  BULLY is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "BULLY server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("BULLY"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of BULLY") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart BULLY to complete") << "\n";
